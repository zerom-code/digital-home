 ...
 *
 * v2 — не из-за формата, а чтобы выбросить уже испорченные кеши с телефонов.
 * До исправления сюда попадал пустой список семей, полученный запросом без
 * токена (useHousehold.ts), и приложение при каждом запуске предлагало
 * создать дом заново. Новые запросы такого больше не сохранят, но у тех, кто
 * успел словить, запись лежала бы в IndexedDB ещё неделю — смена версии
 * стирает её сразу.
 */
