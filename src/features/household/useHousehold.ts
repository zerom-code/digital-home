 ...
 * старте.
 *
 * Кешу этот запрос не верит на слово: `staleTime: 0` и `refetchOnMount:
 * 'always'` заставляют спрашивать сервер при каждом запуске. Для остальных
 * запросов пятиминутная свежесть из lib/query/client.ts правильна — данные о
 * квартире меняются редко. Но именно по этому ответу решается, показать дом
 * или предложить создать новый, и устаревшее «семей нет» из IndexedDB стоит
 * несравнимо дороже одного лишнего запроса. Кешированный ответ при этом никуда
 * не девается: пока новый не пришёл, показываем старый, а офлайн так и
 * останемся на нём.
 */
 ...
 *
 * **Почему «семей нет» — опасный ответ.** По нему App.tsx решает показать
 * онбординг и защёлкивает это решение. Ошибиться тут дороже, чем кажется:
 * человек не просто видит лишний экран, а заводит второй дом поверх
 * существующего, и так при каждом запуске. Именно это и происходило —
 * пустой список приезжал сразу тремя путями:
 *
 * 1. **Гонка с восстановлением сессии.** Хук вызывается раньше, чем
 *    supabase-js достанет токен из хранилища (это асинхронно). Запрос без
 *    токена — под RLS не ошибка, а честный пустой список: не вошёл никто.
 * 2. **Испорченный кеш.** Персистер (lib/query/client.ts) сохранял такой
 *    пустой ответ в IndexedDB как последний известный результат, и при
 *    следующем запуске он подавался уже как готовые данные.
 * 3. **Неудачный запрос.** Сеть отвалилась — `data` нет, и «нет данных»
 *    было неотличимо от «нет семей».
 *
 * Против первого — пауза запроса (`enabled`), пока useSession не ответит.
 * Против второго — `staleTime: 0` и `refetchOnMount: 'always'` (см. выше).
 * Против третьего — `isConfirmed` ниже.
 *
 * `isConfirmed` — это «сервер ответил в этот запуск, и ответ можно
 * использовать для необратимых решений». Одного `!isLoading` мало: он
 * false и когда запрос на паузе, и когда показывается старый кеш, и когда
 * всё упало с ошибкой. `isFetchedAfterMount` же становится true только
 * после настоящего ответа, полученного уже в этом запуске приложения.
 */
export function useActiveHousehold() {
  ...
  return {
    ...
    /** Ответ сервера получен в этот запуск — на него можно опираться. */
    isConfirmed: sessionReady && memberships.isFetchedAfterMount && !memberships.isError,
